...
export async function GET(request) {
  try {
    ...
    console.log(
      "🔍 GET /api/bookings - Fetching bookings for user:",
      userId,
      "role:",
      role
    );
    ...
    if (role === "provider" || role === "all" || !role) {
      ...
      const enhancedProviderBookings = providerBookings.map((booking) => ({
        ...
        isExpired:
          ...
          booking.scheduledPickupTime < new Date() &&
          booking.status !== "collected",
        timeUntilPickup: booking.scheduledPickupTime
          ? Math.max(0, booking.scheduledPickupTime.getTime() - Date.now())
          : null,
      }));
    ...
    if (role === "recipient" || role === "all" || !role) {
      ...
      const recipientBookings = await Booking.find(recipientQuery)
        .populate({
          path: "listingId",
          select:
            "title category imageUrl location expiryTime quantity unit providerId providerName",
        })
        .sort({ createdAt: -1 })
