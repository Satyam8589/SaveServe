...
export async function GET(request) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: 'Internal server error while fetching profile.' },
      { status: 500 }
    );
  }
}
...
export async function POST(request) {
  try {
    ...
    const requiredFields = ['userId', 'fullName', 'email', 'role', 'subrole', 'phoneNumber', 'campusLocation'];
    ...
    if (body.subrole === 'NGO' && (!body.organizationName || body.organizationName.trim() === '')) {
    ...
    const validSubroles = ['CANTEEN', 'HOSTEL', 'EVENTORGANIZER', 'STUDENT', 'STAFF', 'NGO'];
    ...
    const profileData = {
      ...
      organizationName: body.organizationName ? body.organizationName.trim() : '',
