...
const RoleIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
    <path fillRule="evenodd" d="M10 2a1 1 0 00-1 1v1a1 1 0 002 0V3a1 1 0 00-1-1zM4 4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2H4zm1 6a1 1 0 011-1h1a1 1 0 110 2H6a1 1 0 01-1-1zm3 0a1 1 0 011-1h5a1 1 0 110 2H9a1 1 0 01-1-1z" clipRule="evenodd" />
  </svg>
);
...
const UserProfileForm = ({ userId, userEmail, userName, userMainRole, userSubrole, onProfileSaved, onClose }) => {
      ...
      <div className="w-full max-w-xl bg-gray-800/95 border border-gray-700/50 backdrop-blur-xl rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto animate-scale-in">
        ...
        <div className="p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="group">
              <label htmlFor="fullName" className="block text-xs font-semibold text-gray-300 mb-1">Full Name *</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="group">
                <label htmlFor="phoneNumber" className="block text-xs font-semibold text-gray-300 mb-1">Phone Number *</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400">
                    <PhoneIcon />
                  </div>
              ...
              <div className="group">
                <label htmlFor="campusLocation" className="block text-xs font-semibold text-gray-300 mb-1">Campus Location *</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400">
                    <LocationIcon />
                  </div>
            ...
            {formData.subrole === 'NGO' && (
              <div className="group animate-fade-in">
                <label htmlFor="organizationName" className="block text-xs font-semibold text-gray-300 mb-1">Organization Name *</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400">
                    <BuildingIcon />
                  </div>
            ...
            {/* Description Field */}
            <div className="group">
              <label htmlFor="description" className="block text-xs font-semibold text-gray-300 mb-1">Tell us about yourself</label>
              <div className="relative">
                <textarea
                  name="description"
                  id="description"
          ...
          {status.message && (
            <div className={`mt-4 p-3 rounded-lg text-center font-medium text-sm transition-all duration-300 animate-fade-in ${
              status.type === 'success' 
...
const ProfilePage = () => {
  ...
  const userMainRole = user?.publicMetadata?.mainRole || user?.unsafeMetadata?.mainRole;
  const userSubrole = user?.publicMetadata?.subRole || user?.unsafeMetadata?.subRole;
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Profile not found (404) - showing modal to create profile');
        setProfileData(null);
        setShowModal(true);
      } else {
  ...
  useEffect(() => {
    console.log('ProfilePage useEffect - isLoaded:', isLoaded, 'isSignedIn:', isSignedIn, 'userId:', userId);
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center">
        <div className="text-center animate-fade-in">
          ...
          {userId && <p className="text-sm text-gray-500 mt-2">User ID: {userId.substring(0, 8)}...</p>}
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between animate-fade-in">
            <div>
              ...
              <p className="text-gray-300 mt-2 text-lg">Manage your information and preferences</p>
            </div>
        ...
        {profileData ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-gray-800/80 border border-gray-700/50 backdrop-blur-xl rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div className="relative bg-gradient-to-r from-emerald-600 via-orange-600 to-amber-600 p-8 text-center overflow-hidden">
                  ...
                  <div className="relative z-10 animate-scale-in">
                    ...
                    <h2 className="text-3xl font-bold text-white mb-2">{profileData.fullName}</h2>
                    <p className="text-white/80 text-lg mb-4">{profileData.email}</p>
                    <div className="flex justify-center space-x-3 mt-4">
                      <span className={`px-4 py-2 rounded-full text-sm font-medium bg-gradient-to-r ${getRoleBadgeColor(profileData.role)} text-white`}>
                        {profileData.role}
                      </span>
                      <span className={`px-4 py-2 rounded-full text-sm font-medium bg-gradient-to-r ${getSubroleBadgeColor(profileData.subrole)} text-white`}>
                        {profileData.subrole}
                      </span>
                    </div>
                ...
                <div className="p-8 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                      { icon: PhoneIcon, label: "Phone Number", value: profileData.phoneNumber, color: "blue" },
                      { icon: LocationIcon, label: "Campus Location", value: profileData.campusLocation, color: "purple" },
                      { icon: RoleIcon, label: "Main Role", value: profileData.role, color: "green" },
                      { icon: RoleIcon, label: "Subrole", value: profileData.subrole, color: "indigo" }
                    ].map((item, index) => (
                      <div
                        key={index}
                      ...
                      >
                        <div className={`flex-shrink-0 w-10 h-10 bg-${item.color}-500/20 rounded-lg flex items-center justify-center`}>
                          <item.icon />
                        </div>
                        <div>
                          <h3 className="text-sm font-semibold text-gray-300">{item.label}</h3>
                          <p className="text-lg text-white mt-1">{item.value || 'Not provided'}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {profileData.organizationName && (
                    <div className="flex items-start space-x-3 p-4 bg-gray-700/50 rounded-xl hover:bg-gray-700/70 transition-all duration-200 border border-gray-600/30 transform hover:scale-105 hover:-translate-y-1 animate-fade-in-up">
                      ...
                      <div>
                        ...
                        <p className="text-lg text-white mt-1">{profileData.organizationName}</p>
                      </div>
                    </div>
                  )}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-800/80 border border-gray-700/50 backdrop-blur-xl rounded-2xl shadow-2xl p-6 animate-fade-in-right">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${profileData.isProfileComplete 
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${profileData.isActive 
                      ? 'bg-green-500/20 text-green-300 border border-green-500/30' 
              ...
              <div className="bg-gray-800/80 border border-gray-700/50 backdrop-blur-xl rounded-2xl shadow-2xl p-6 animate-fade-in-right" style={{ animationDelay: '200ms' }}>
                ...
                <div className="space-y-4">
                  {[
                    { label: "Profile Created", date: profileData.createdAt, color: "blue" },
                    { label: "Last Updated", date: profileData.updatedAt, color: "green" },
                    { label: "Last Login", date: profileData.lastLoginAt, color: "purple" }
                    ...
                    <div 
                      ...
                      style={{ animationDelay: `${index * 100}ms` }}
                    >
                      <div className={`flex-shrink-0 w-8 h-8 bg-${item.color}-500/20 rounded-full flex items-center justify-center`}>
                        <CalendarIcon />
                      </div>
                      <div>
              ...
              <div className="bg-gray-800/80 border border-gray-700/50 backdrop-blur-xl rounded-2xl shadow-2xl p-6 animate-fade-in-right" style={{ animationDelay: '400ms' }}>
                ...
                <div className="space-y-3">
                  {[
                    { icon: EditIcon, label: "Edit Profile", onClick: () => setShowEditForm(true) },
                    { icon: () => <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>, label: "Privacy Settings", onClick: () => {} },
                    { icon: RefreshIcon, label: "Refresh Data", onClick: handleRefresh },
                    { icon: () => <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>, label: "Help & Support", onClick: () => {} }
                  ].map((action, index) => (
                    <button 
                      key={index}
                      onClick={action.onClick}
                      className="w-full text-left px-4 py-3 bg-gray-700/50 hover:bg-gray-700/70 rounded-xl transition-all duration-200 flex items-center space-x-3 border border-gray-600/30 hover:border-gray-600/50 transform hover:scale-105 hover:-translate-y-1 animate-fade-in"
                      style={{ animationDelay: `${index * 100}ms` }}
                    >
                      <action.icon />
              ...
              {/* Fun Stats */}
              <div className="bg-gray-800/80 border border-gray-700/50 backdrop-blur-xl rounded-2xl shadow-2xl p-6 animate-fade-in-right" style={{ animationDelay: '600ms' }}>
          ...
          <div className="text-center py-12 animate-fade-in">
            <div className="max-w-md mx-auto bg-gray-800/80 border border-gray-700/50 backdrop-blur-xl rounded-2xl shadow-2xl p-8">
              ...
              <p className="text-gray-300 mb-6">Please complete your profile to get started with all the features.</p>
              {error && (
