...
const QRCodeDisplay = ({ booking, onClose }) => {
  ...
  if (!booking) {
    return (
      <Card className="w-full max-w-md mx-auto">
        ...
        <CardContent>
          <p>Unable to load booking information. Please try again later.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  // Use the enriched booking data passed from the main page with safety checks
  ...
  const downloadQRCode = () => {
    if (!booking?.qrCodeImage) {
      console.error("No QR code image available");
      return;
    }
    const link = document.createElement("a");
    link.href = booking.qrCodeImage;
    link.download = `food-collection-qr-${booking?._id || "unknown"}.png`;
    link.click();
  };
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="bg-gray-700/50 p-3 rounded text-xs space-y-1">
                ...
                <div className="flex justify-between">
                  ...
                  >
                    {expiryTime?.toLocaleString() || "Unknown"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">
                ...
                {booking.collectionInstructions?.steps?.map((step, index) => (
                  ...
                  <>
                    ...
                    <li className="flex items-start">
                      ...
                      {expiryTime?.toLocaleTimeString() || "Unknown"}!
                    </li>
                  </>
                )}
              </ul>
