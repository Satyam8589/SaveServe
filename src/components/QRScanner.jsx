...
const QRScanner = ({ onScanSuccess, onClose, listingId, providerId }) => {
  ...
  const handleQRDetection = useCallback(
    async (qrData) => {
      ...
      setScanError(null);
      try {
        const result = await verifyCollectionMutation.mutateAsync({
          qrData,
          providerId,
          listingId,
        });
        // Pass the verified booking data to the success handler
        onScanSuccess(result.data?.booking || result.booking || { _id: null });
      } catch (error) {
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      setScanError(
        "Failed to start camera. It might be in use by another application."
      );
    }
  }, [facingMode, tick]);
          ...
          <div className="space-y-2 pt-4 border-t border-gray-600">
            ...
            <div className="flex space-x-2">
              ...
              <Button
                ...
                disabled={
                  manualCode.length !== 6 || verifyCollectionMutation.isPending
                }
                className="bg-emerald-600 hover:bg-emerald-700"
              >
                Verify
